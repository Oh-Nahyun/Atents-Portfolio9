...
public class Crosshair : MonoBehaviour
{
    ...
    /// <summary>
    /// 기본적으로 확장되어있는 길이 (부모에서 기본적으로 떨어져있는 정도)
    /// </summary>
    const float defaultExpend = 10.0f;

    /// <summary>
    /// 현재 확장되어있는 길이 (defaultExpend에서 얼마나 더 확장되었는지)
    /// </summary>
    float current = 0.0f;
    ...
    /// <summary>
    /// 나누기를 자주하는 것을 피하기 위해 미리 계산해놓은 것
    /// </summary>
    const float divPreComput = 1 / recoveryDuration;

    /// <summary>
    /// 4방향 Crosshair 이미지의 트랜스폼들
    /// </summary>
    RectTransform[] crossRects;

    /// <summary>
    /// 각 Crosshair 이미지의 이동 방향
    /// </summary>
    readonly Vector2[] direction = { Vector2.up, Vector2.right, Vector2.down, Vector2.left };

    private void Awake()
    {
        crossRects = new RectTransform[transform.childCount]; // Crosshair의 이미지를 미리 찾아놓기
    ...
    public void Expend(float amount)
    {
        ...
        StopAllCoroutines();                                // 코루틴 중복 실행을 방지하기 위해 실행되던 코루틴 정지
    ...
    IEnumerator DelayRecovery(float wait)
    {
        ...
        {
            curveProcess += Time.deltaTime * divPreComput; // recoveryDuration 기간에 맞춰서 curveProcess 진행
            current = recoveryCurve.Evaluate(curveProcess) * startExpend; // current 계산하기 (커브 결과 * 최대치)
            for (int i = 0; i < crossRects.Length; i++)
            {
