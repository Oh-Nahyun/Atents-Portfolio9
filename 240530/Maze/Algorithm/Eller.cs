...
public class Eller : Maze
{
    ...
    protected override void OnSpecificAlgorithmExcute()
    {
        ...
        ///      2.2. 서로 같은 집합이면 패스
        /// 3. 아래쪽 벽 제거하기
        ///      3.1. 같은 집합 당 최소 하나 이상의 벽이 제거되어야 한다.
        /// 4. 한 줄 완료 -> 1번으로 돌아가기
        /// 5. 마지막 줄 만들고 정리
        ///      5.1. 생성까지는 똑같이 처리
        ///      5.2. 합칠 때 세트가 다르면 무조건 합친다.

        int h = height - 1;             // 미리 계산해놓기
        EllerCell[] prevLine = null;    // 이전 줄을 저장하는 변수

        for (int y = 0; y < h; y++)     // 첫 줄부터 마지막 줄까지 만드는 용도의 for문
        {
            // 1. 한 줄 만들기
        ...
        const float LastMergeChance = 1.1f;         // 마지막 줄에서 이웃 셀과 합쳐질 확률 (무조건 합쳐져야 함)
    ...
    {
        int count = 1;      // 한 줄이 모두 같은 집합에 속하는 것을 방지하기 위한 카운터
        int w = width - 1;  // 미리 계산해놓을 것
        for (int x = 0; x < w; x++)
        {
            if (count < w && line[x].setGroup != line[x + 1].setGroup && Random.value < chance)
            {
                // count가 width보다 작다 = 한 줄이 모두 같은 집합에 속하지 않는다.
                // x와 x + 1번째의 셀이 같은 그룹에 속하지 않는다.
                // 설정한 확률을 통과했다.

                line[x].MakePath(Direction.East);           // 서로 길 만들기
                line[x].MakePath(Direction.West);

                int targetGroup = line[x + 1].setGroup;     // x+1번째의 집합을 저장해놓기
    ...
    /// </summary>
    /// <param name="line">처리할 줄</param>
    void RemoveSouthWall(EllerCell[] line)
    {
        // 집합 별로 리스트 만들기
        // 키 : 집합 번호, 값 : 이 줄의 셀 중 키값에 해당하는 집합에 포함되는 셀의 x좌표
