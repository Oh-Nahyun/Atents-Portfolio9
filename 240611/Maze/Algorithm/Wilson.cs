...
public class Wilson : Maze
{
    ...
    protected override void OnSpecificAlgorithmExcute()
    {
        ...
        /// 3. A셀의 위치에서 랜덤으로 한 칸 움직인다. (이동한 셀이 기록되어야 한다.)
        ...
        List<int> notInMaze = new List<int>(notInMazeArray); ///// 배열을 기반으로 한 리스트 만들기
        ...
        while (notInMaze.Count > 0)                             // notInMaze에 포함된 아이템의 개수가 0보다 크다 == 아직 미로에 포함되지 않은 셀이 남아있다.
        {
            ...
            do
            {
                WilsonCell neighbor = GetNeighbor(current);         // 이웃 셀을 구하고
                current.next = neighbor;                            // 어디로 이동하는지 기록
                current = neighbor;                                 // current 변경
            } while (!current.isMazeMember); // 4. 미로에 포함된 셀에 도착할 때까지 3번을 반복한다.
            ...
            while (path != current) // 시작부터 current에 도착할 때까지 돌기
            {
                ...
                notInMaze.Remove(GridToIndex(path.X, path.Y));      // 미로에 포함되어있지 않은 셀들의 목록에서 제거
